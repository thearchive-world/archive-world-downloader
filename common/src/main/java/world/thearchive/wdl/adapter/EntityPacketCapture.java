 ...
 * still-separate {@code AddExperienceOrb} as the spawn payload (the orb folds into {@code AddEntity} at 1.21.5), the
 * synced data values as the synced-value payload, and the equipment slot/stack pair as the equipment payload. The
 * reconstruct applies each post-spawn packet the way the client handlers do, so this only has to decode each packet to
 * the same state the client would hold.
 *
