...
/**
 * 1.10.2 lectern sink: serializes a lectern's book via vanilla's own {@code ItemStack.writeToNBT} and merges it into a
 * captured lectern block-entity tag.
 *
 * <p>Documented limit: the lectern is a 1.14 block, absent at this band (it does not exist even at the 1.11.2 mint
 * parent), so no lectern block entity is ever captured and this sink never runs against real data. It stays wired as a
 * compiling no-op behind the band-stable SPI, the way the pre-1.14 crafter and other later features stay as
 * unused-but-compiling plugs, so the shared capture path does not fork on the band.
 *
