 ...
 * hands over a lazy encode {@link Supplier} that the writer thread resolves (the deferred but deterministic encode of
 * immutable, detached inputs); {@link #submitEntity} hands over a fully-encoded, immutable {@link CompoundTag}. Either
 * way the writer thread is the sole owner of the {@link SimpleRegionStorage}s, the one-writer invariant, and the reason
 * only immutable or deferred-immutable work may cross the queue.
 *
 ...
 * open lazily by get-then-put, so a second writer could open two storages over one directory. The merge path is
 * read-modify-write per {@link ChunkPos} and would lose a merge if interleaved. And the finalize order is fixed.
 *
...
final class AsyncSaveWriter {
    ...
    /** The best-effort pre-write step run before the drain opens any storage. */
    @FunctionalInterface
    ...
    /**
     * The on-disk step run on the writer thread after the storages are drained and closed (the band's level.dat write).
     * It receives the writer's soft-failure tallies, so a step that records how the download went can stamp the real
    ...
    /**
     * The best-effort finish-time output step run after the storages and session lock have closed. Returns the written
     * file's name for the completion surfaces, or null when none was written.
     */
    @FunctionalInterface
     ...
     * {@link #chunksWritten()} is the total the saved-world message reports. {@code zipFileName} is the finish-time
     * output actually written on a clean save, or null (none configured, the step failed, or the save failed), so a
     * completion surface never names a file that is not on disk.
     */
    record SaveResult(int chunksNew, int chunksRecaptured, int chunksFailed, int entityChunksWritten,
            int entityChunksFailed, @Nullable String zipFileName, @Nullable Throwable error) {
    ...
    private sealed interface Task permits WriteTask, FinalizeTask {}
    ...
    // Total write tasks handed over so far, the denominator of the drain's progress fraction. Incremented on
    // the main thread at submit and read on the writer thread, hence volatile; only the main thread writes it.
     ...
     * written into the folder, {@code finalizer} after the drain while the folder is open (the band's level.dat write),
     * and {@code outputs} after the storages and session lock have closed. {@code preflight} and {@code outputs} are
     * best-effort: a throw from either is caught and logged, never failing the save, so neither can endanger the
     * openable folder. A {@code finalizer} throw aborts the save the usual way (it is the level.dat write).
     */
    ...
    /**
     * Enqueue a lazy encode thunk for {@code dimension}'s {@code region/} storage (main thread). The writer thread
     * resolves {@code encode} (the heavy chunk serialize), so it must close over only immutable, detached inputs the
     * writer then solely owns (the snapshot, the per-band codec, the frozen registries). {@code merge} is the on-disk
     * read-merge the writer applies to that chunk, supplied per chunk because what a prior copy contributes is the
     * caller's decision and belongs with the chunk it is decided for.
     */
    ...
    public void submitEntity(ResourceKey<Level> dimension, ChunkPos pos, CompoundTag tag) {
        submitted++;
        // An entity chunk is written whole, so a prior copy of the position says nothing this one does not.
        queue.add(new WriteTask(Target.ENTITIES, dimension, pos, () -> tag, (onDisk, fresh) -> 0));
    }
    ...
    private void run() {
        ...
        try {
            bestEffort(preflight, "the pre-write step");
            long drained = 0;
            Task next;
            while (!((next = queue.take()) instanceof FinalizeTask)) {
                ...
                CompoundTag tag;
                try {
                    tag = task.encode().get(); // the encode runs on this thread (a no-op for entities)
                ...
                if (task.target() == Target.REGION) {
                    ...
                    // A fresh download reads empty for most chunks, but not for all of them: a resume writes into
                    // a folder that already holds a prior copy of the position, which is why the merge is supplied
                    // per chunk rather than fixed here.
                    ...
                    switch (merged.outcome()) {
                        ...
                        case WRITTEN_RECAPTURED -> chunksRecaptured++;
                    ...
                    switch (merged.outcome()) {
                        case WRITTEN_NEW, WRITTEN_RECAPTURED -> entityChunksWritten++;
            ...
            finalizer.run(chunksFailed, entityChunksFailed); // level.dat
        ...
        // The finalize-time output runs only after the folder is fully written and closed, and only on a clean
        // save: a failed save must not be published. Best-effort, so its failure never turns a successful save
        // into a reported failure.
        String zipFileName = null;
        if (error == null) {
            zipFileName = bestEffortOutput(outputs, "the finish-time output");
        }
        result.complete(new SaveResult(chunksNew, chunksRecaptured, chunksFailed, entityChunksWritten,
                entityChunksFailed, zipFileName, error));
    }

    /** Run a best-effort writer-thread step; a throw is logged, never fatal. */
    ...
    /** As {@link #bestEffort}, for the output step: a throw is logged and reports no written file. */
