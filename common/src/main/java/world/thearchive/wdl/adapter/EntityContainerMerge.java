 ...
 * {@link ContainerSink#merge} (a chest or hopper minecart persists its contents under {@code "Items"} in the shape the
 * sink writes), so the two classes share the serialize and differ only in the locator.
 *
 * <p>The same UUID locator also folds a villager's captured trades ({@link #mergeMerchantStash} and its refold): those
 * set {@code "Offers"} from a holder the tick already serialized, a plain tag copy with no sink and no per-entry
 * failure isolation, so unlike the item fold they cannot throw and always report zero failed.
 *
...
final class EntityContainerMerge {
     ...
     * sibling of {@link #refoldFlushedContainers}, so a villager that crossed entity-chunks carries its trades to each
     * copy. A node whose menu was reopened is skipped, since {@link #mergeMerchantStash} is about to fold its fresher
     * capture. Like the merge above it cannot fail and reports zero failed.
     */
    ...
    /**
     * Copy the holder's {@code "Offers"} onto the node, and its {@code "Xp"} when present (trade experience is a 1.14
     * addition, so no holder below it carries one).
     */
