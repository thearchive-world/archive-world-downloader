 ...
 * yet never z-fights a flush one. Each edge carries its own direction as its normal, because the lines shader thickens
 * perpendicular to position plus normal, so a face-outward normal would collapse the edge to zero width. We draw the
 * edges explicitly rather than feed a flat {@code VoxelShape} to {@code renderShape}, which risks degenerate
 * zero-length edges.
 */
