 ...
 * the {@code {Entities, Position, DataVersion}} envelope from already-serialized entity tags and is what the headless
 * round-trip exercises. A serialized-entity {@link CompoundTag} is the entity analog of {@link ChunkSnapshotSource}, so
 * the pure step takes a {@code List<CompoundTag>}.
 *
...
public interface EntitySink {
     ...
     * {@link Entity#shouldBeSaved()} gate {@link #encodeChunk(List, ChunkPos, boolean)} applies, because a vehicle
     * carrying exactly one player fails that gate by design. Leash-safe over the vehicle and its passengers; the player
     * passenger self-excludes ({@code saveAsPassenger} refuses a type with no encode id). A mount dismounted in the
     * downloaded world despawns like any other captured mob, so it takes the same {@code PersistenceRequired}
     * restoration the entities path applies: a named mob and a loot-equipped mob are stamped unconditionally (the
     * client entity never carries the server flag, so it would despawn on open), and with {@code forceMobPersistence}
     * set every captured mob is.
     *
