 ...
 * items, a drained container), a chunk-path block entity via {@link #scrubBlockEntity(NBTTagCompound)}, and a
 * serialized entity via {@link #scrubEntity(NBTTagCompound)} (an item frame, mob equipment, a dropped item, and their
 * passengers).
 *
    ...
    private static void scrubItemTag(NBTTagCompound tag) {
        ...
        if (tag.getTag(BLOCK_ENTITY_TAG) instanceof NBTTagCompound) {
            ...
            if (blockEntityTag.getTag(BEES) instanceof NBTTagList) {
                ...
                for (NBTBase element : NbtElements.of(bees)) {
