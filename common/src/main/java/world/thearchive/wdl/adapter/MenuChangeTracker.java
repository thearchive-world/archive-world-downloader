 ...
 * {@code ContainerData} vector (a brewing stand's brew time and fuel), so a data-only tick like a running brew with no
 * slot movement re-stashes. Main-thread only, reset at bind and close so one open menu never inherits another's
 * snapshot.
 */
