 ...
 * case, replaces the stack's {@code minecraft:bundle_contents} value ({@code BundleItem}'s click-prediction override
 * mutates the slot-resident stack, and the server's matching ack resends no slot; {@code BundleContents} is immutable
 * and swapped wholesale on every insert or remove, so a reference compare catches it). {@code BundleItem} is the only
 * vanilla overrider of the stack-on-stack click hooks, so no other component mutates in a slot. The signal also carries
 * a small menu-only {@code ContainerData} vector (a crafter's disabled and triggered flags, a brewing stand's brew time
 * and fuel with their totals), so a data-only tick like a running brew with no slot movement re-stashes. Main-thread
