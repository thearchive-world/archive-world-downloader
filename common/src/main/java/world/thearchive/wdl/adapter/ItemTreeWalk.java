 ...
 * {@code Damage}, not the inner {@code tag}, so the map-id pass takes the item view; a filled map carries no
 * {@code tag} at all and the tag view would never see it. Each caller supplies its own per-item leaf action.
 */
