...
final class PlayerTag {
     ...
     * ({@code targetDimension}), so it is already one of the three {@link VanillaDimensions#forType} returns. At 1.15.2
     * vanilla reads this back as the integer id ({@code Entity.load}), so it is written as the id, not the 1.16 string
     * form: a string here is read by {@code getInt} as 0 and lands the player in the overworld.
     */
     ...
     * writes) and {@code "Entity"} is the root vehicle serialized standalone. Mirrors vanilla's player save, whose
     * record its player load reads on open (a singleplayer saved player included) to respawn the vehicle and re-seat
     * the player; an absent record is a clean load that leaves the player standing.
     */
