 ...
 * sibling of {@link ChunkMerge}. The saddle needs no carry-forward key on either side of the 1.21.5 equipment-slot cut:
 * above it the saddle rides in the synced equipment every capture re-reads, and below it capture re-derives
 * {@code "SaddleItem"} from the synced saddled flag on every pass, so only {@code "Items"} carries forward.
 */
