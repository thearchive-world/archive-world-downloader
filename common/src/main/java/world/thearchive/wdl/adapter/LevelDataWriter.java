 ...
 * regenerated terrain. The generator is stored as a {@code LevelType} plus its options in level.dat, so no worldgen
 * registries are needed to write it.
 */
public interface LevelDataWriter {
    ...
    /**
     * Reconstruct the client-side worldgen ahead of time, off the render thread. The default is a no-op: the captured
     * VOID/DEFAULT/FLAT world stores only a generator name and options and needs no worldgen reconstruction, so no band
     * below the 1.16 registry rework does anything in it.
     */
     ...
     * across bands: the write is {@code LevelStorage.saveLevelData(LevelData, playerTag)}, where the modern bands write
     * through {@code LevelStorageAccess.saveDataTag}.
     *
    ...
    /**
     * The prior download's captured player tag, read from its on-disk home given the prior {@code levelDatFile}, or
     * null when the folder is fresh or no player was written. This is the read mirror of {@link #save}'s player write:
     * the player is a {@code level.dat "Player"} compound. A resume consumes this to carry the prior ender chest and
     * mount contents forward without the player reopening them. A present but unreadable file throws (the caller
     * degrades it to a skipped carry-forward, fail-soft).
     */
    @Nullable
