 ...
 * installed, so the class never loads without it; the ModMenu API is compile-only and never ships. ModMenu 1.7.x, the
 * 1.14.4 line, keys each provider by {@link #getModId()} in its {@code io.github.prospector.modmenu.api.ModMenuApi} and
 * takes the parent-to-screen factory from {@link #getConfigScreenFactory()}.
 */
