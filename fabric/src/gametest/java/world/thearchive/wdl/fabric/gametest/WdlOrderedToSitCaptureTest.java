...
public class WdlOrderedToSitCaptureTest implements FabricClientGameTest {
    ...
    private static boolean sitting(CompoundTag entity) {
        return entity.getBoolean("Sitting");
    }

    private static CompoundTag findCat(List<CompoundTag> entities, String customName) {
        return entities.stream()
                .filter(entity -> entity.getString("id").equals("minecraft:cat"))
    ...
    private static CompoundTag findBoat(List<CompoundTag> entities) {
        return entities.stream()
                .filter(entity -> entity.getString("id").equals("minecraft:oak_boat"))
                .findFirst()
    ...
    private static List<CompoundTag> passengers(CompoundTag vehicle) {
        return vehicle.getList("Passengers", Tag.TAG_COMPOUND).stream().map(tag -> (CompoundTag) tag).toList();
    }

    /**
     * The custom name a mob was captured with: pre-1.21.5 writes CustomName as a serialized JSON text component, so a
     * plain literal arrives quoted and an object carries it under text.
     */
