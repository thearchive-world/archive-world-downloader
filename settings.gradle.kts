...
// NeoForge has no release below 1.20.1, so the loader here is Forge, built by a hand-rolled plain-java island under
// forge/ (a separate Gradle build with no ForgeGradle and no Loom: it compiles against the Mojmap Minecraft plus a
// Mojmap-named view of the real Forge universal jar forge_version names, reobfuscates Mojmap -> SRG by hand, and
// packages). This root builds only common; the Forge jar is produced by forge/gradlew, not from here.
include("common")
