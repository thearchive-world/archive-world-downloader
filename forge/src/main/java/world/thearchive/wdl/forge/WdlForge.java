...
public final class WdlForge {
    public WdlForge() {
        ...
        // The config-screen factory is an ExtensionPoint carrying a BiFunction, not the factory type the
        // 1.17-and-above bands register by its class.
        ...
        MinecraftForge.EVENT_BUS.addListener((PlayerInteractEvent.RightClickBlock event) -> {
            // RightClickBlock carries no hit vector; getHitVec is a 1.16 addition. The consumers read only the
            // clicked block position, so the hit is rebuilt from the event's position and face, with the block
            // center standing in for the precise location the event does not provide.
        ...
        // There is no OverlayRegistry (a 1.17 addition) or RegisterGuiOverlaysEvent (1.19); the HUD host is
        // RenderGameOverlayEvent on the game bus. Post with ElementType.ALL fires once after the whole vanilla HUD,
        // so the overlay draws above every element in every gamemode; it self-gates F1 and blocking screens.
